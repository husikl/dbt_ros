...
from asar_planner_visual_servoing.srv import PlanWithPddl, PlanWithPddlResponse, PlanWithPddlRequest
# , GetObjectsInfo
...
# some_xml_data = '<BehaviorTree ID="MoveRobot"><Fallback><Sequence><MoveBase       goal="{target}"/><SetBlackboard output_key="output" value="mission accomplished" /></Sequence></Fallback></BehaviorTree>'
...
def getEffects(action, a_list):
    ...
    if action_list[0] == str("place") or action_list[0] == str("pick"):
        ...
        for e in action.effects_xml:
            ...
            if 'pose' in e_xml.attrib :
                e_xml.attrib['pose'] = str(objects[0].coordinates.positon.x) + ';'+ str(objects[0].coordinates.positon.y)+';0' 
            if 'at' in e_xml.attrib :
                e_xml.attrib['at'] = str(objects[1].coordinates.positon.x) + ';'+ str(objects[1].coordinates.positon.y)+';0' 
        ...
        for e in action.effects_xml:
            ...
            if 'pose' in e_xml.attrib :
                e_xml.attrib['pose'] = str(obj.coordinates.position.x) + ';'+ str(obj.coordinates.position.y)+';0' 
            effs.append(e_xml)
...
def getActionPrecondtions(action, a_list):
    ...
    if action_list[0] == str("place") or action_list[0] == str("pick") :
        ...
        for c in action.precond:
            ...
            if 'pose' in c_xml.attrib :
                c_xml.attrib['pose'] = str(objects[1].coordinates.position.x) + ';'+ str(objects[1].coordinates.position.y)+';0' 
        ...
        for c in action.precond:
            ...
            if c.type == str("is_object_detected"):
                ...
                if 'pose' in c_xml.attrib :
                    c_xml.attrib['pose'] = str(objects.coordinates.position.x) + ';'+ str(objects.coordinates.position.y)+';0' 
            conds.append(c_xml)
...
def getActionFromPddl(action, a_list):
    ...
    if action_list[0] == str("place") or action_list[0] == str("detect") or action_list[0] == str("pick")  :
        ...
        if 'pose' in action_xml.attrib :
            action_xml.attrib['pose'] = str(objects[1].coordinates.position.x) + ';'+ str(objects[1].coordinates.position.y)+';0' 
        ...
        if 'at' in action_xml.attrib :
            action_xml.attrib['at'] = str(objects[1].coordinates.position.x) + ';'+ str(objects[1].coordinates.position.y)+';0' 
        ...
        if 'pose' in action_xml.attrib :
            action_xml.attrib['pose'] = str(object.coordinates.position.x) + ';'+ str(object.coordinates.position.y)+';0' 
        if 'at' in action_xml.attrib :
            action_xml.attrib['at'] = str(object.coordinates.positon.x) + ';'+ str(object.coordinates.positon.y)+';0' 
    return action_xml

def extract_numbers(s):
    # print("s ~ ", s)
    # split the string into two parts using the underscore character
    # parts = s.split('_')
    
    # # extract the two numbers as integers
    # num1 = int(parts[0][1:])
    # num2 = int(parts[1])
